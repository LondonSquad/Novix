...
class FaceDetector(private val context: Context) {
    ...
    suspend fun detectFaces(bitmap: Bitmap): List<DetectedFace> = withContext(Dispatchers.IO) {
        val interpreter = this@FaceDetector.interpreter
        if (interpreter == null) {
        ...
        try {
            ...
            val needsEnhancement = needsEnhancement(bitmap)
            ...
            val finalFaces = enhancedNMS(allFaces).take(MAX_FACES)
    ...
    private fun needsEnhancement(bitmap: Bitmap): Boolean {
        ...
        return avgBrightness < 80 || avgBrightness > 200 || contrast < 50
    }
