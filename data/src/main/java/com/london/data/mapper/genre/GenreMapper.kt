...
interface GenreMapper {
    ...
    fun List<Int>?.toMovieGenre(): List<MovieGenre> = this?.map { it.toMovieGenre() }.orEmpty()

    fun List<Int>?.toTvShowGenre(): List<TvShowGenre> = this?.map { it.toTvShowGenre() }.orEmpty()

    fun List<Int>?.toGenre(mediaType: MediaType): List<Genre> =
        this?.map { it.toGenre(mediaType) }.orEmpty()

    private fun Int.toMovieGenre(): MovieGenre = when (this) {
    ...
    private fun Int.toTvShowGenre(): TvShowGenre = when (this) {
    ...
    private fun Int.toGenre(mediaType: MediaType): Genre = when (mediaType) {
